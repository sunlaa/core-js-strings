...
function formatTime(minutes, seconds) {
  const min = minutes < 10 ? String(minutes).padStart(2, '0') : minutes;
  const sec = seconds < 10 ? String(seconds).padStart(2, '0') : seconds;
  return `${min}:${sec}`;
}
 ...
 */
function getStringFromTemplate(firstName, lastNam) {
  return `Hello, ${firstName} ${lastNam}!`;
}
